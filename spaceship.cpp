...
void Spaceship::CreateShip (std::vector<VAOStruct> vaos,std::vector<Shader> shaders, 
    ...
    this->shapes.push_back(GetRectangle(position_vao, glm::vec3(-.05, 0.0, 0.0), 0.10, 0.4));
...
void Spaceship::Draw(std::vector<unsigned int> textures) {
        ...
        glm::mat4 bullet_transform = glm::mat4(1.0);


        bullet_transform = glm::scale(bullet_transform, glm::vec3(0.5));        
        //bullet_transform = glm::rotate(bullet_transform, glm::radians(this -> angle_z), glm::vec3(0.0, 0.0, 1.0));
        //bullet_transform = glm::translate(bullet_transform, this -> location);
        bullet_transform = glm::translate(bullet_transform, glm::vec3(bullet_loc));
        ...
        std::vector<Shader*> shader_ptrs = {&shader,&shader,&shader,&shader,&texture_shader,&shader, &shader};
...
void Spaceship::Shoot(){
    this -> bullet_loc += glm::vec3(0.2*cos(glm::radians(angle_z)), 0.2*sin(glm::radians(angle_z)), 0.0);
    this -> last_shot = glfwGetTime();
};
